...
public class TileEntityMasterChevron extends TileEntityStargate {
	 ...
	 */
	private static final int N = StargateMod.naquadahAlloy.blockID;
	...
	/**
	 * Retrourne l'angle entre cette porte et la porte de destination.
	...
	/**
	 * Retourne l'axe correspondant a l'angle passe en parametre.
	 * @param angle - un angle (0, 1, 2, ou 3).
	 * @return un axe (X_AXIS ou Z_AXIS).
	 */
	...
	public void teleportEntity(Entity entity) {
		...
		if(!this.teleportedEntities.containsKey(entity.entityId)) {
			...
			// Si l'entite est un minecart, la position et la vitesse doivent etre corrigees.
			...
			// FIXME - La direction/vitesse du Minecart ne semblent pas pouvoir etre controlees correctement...
			//			if(entity instanceof EntityMinecart) {
			//				int otherAngle = this.getOtherAngle();
			...
			// On met de nouveau a jour la vitesse de l'entite.
			StargateMod.sendPacketToAllPlayers(new Packet28EntityVelocity(entity));
	...
	private void playSoundEffect(String sound) {
		this.worldObj.playSoundEffect(this.xCoord, this.yCoord - 7, this.zCoord, sound, 2, 1);
	}
	...
	/**
	 * Construit une porte a l'emplacement indique.
	 * @param world - le monde dans lequel construire la porte.
	 * @param x - la coordonnee en X.
	 * @param y - la coordonnee en Y.
	 * @param z - la coordonnee en Z.
	 * @param side - l'orientation de la porte.
	...
	public static boolean constructGate(World world, int xCoord, int yCoord, int zCoord, int side) {
		// Intialisation.
		StargateMod.debug("Building a gate for test...", true);
		
		// Si la porte est situee trop bas sur l'axe Y.
		if(yCoord < PATERN.length) {
			// Le patern ne peut pas etre respecte.
			StargateMod.debug("La porte est situee trop bas sur l'axe Y !\n", true);
			return false;
		}
		
		// Si le cote est mal choisit.
		if(side < 2 && side > 5) {
			// Le patern ne peut pas etre respecte.
			StargateMod.debug("La porte est mal orientee !\n", true);
			return false;
		}
		...
		for(int i = 0; i < PATERN.length; ++i) {
			...
			for(int j = -demiLargeur + 1; j < demiLargeur; ++j) {
				...
				if(Math.abs(j) >= nbCasesAir) {
					...
					int z = zCoord + (axeZ * j);
					int indexBlock = Math.abs(j) - nbCasesAir + 1;
					int blockId = PATERN[i][indexBlock];
		...
		// On oriente le MasterChevron dans la bonne direction.
		world.setBlockMetadataWithNotify(xCoord, yCoord, zCoord, side);
		
		// Si on est arrive jusqu'ici, c'est que le patern est verifie.
		StargateMod.debug("Construction terminee !\n", true);
		return true;
	}
