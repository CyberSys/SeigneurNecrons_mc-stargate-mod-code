...
public abstract class ModConfig<M extends ModBase> {
	...
	public int getBlockId(String blockName) {
		while(Block.blocksList[this.nextBlockId] != null || this.isIdTaken(CATEGORY_BLOCK, this.nextBlockId, blockName)) {
			this.nextBlockId++;
		}
	...
	public int getItemId(String itemName) {
		while(Item.itemsList[this.nextItemId + 256] != null || this.isIdTaken(CATEGORY_ITEM, this.nextItemId, itemName)) {
			this.nextItemId++;
		}
	...
	protected boolean isIdTaken(String category, int id, String name) {
		Collection<Property> properties = this.config.getCategory(category).values();
		
		for(Property property : properties) {
			if(property.getInt() == id) {
